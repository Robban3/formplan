...
/**
 * Beskuren, INCHECKAD kopia av de ~80 källövningar katalogen faktiskt använder.
 * Utan den hämtade varje CI-körning ~870 kB från raw.githubusercontent.com utan
 * timeout eller omförsök — en 429 eller ett nätverksglapp fällde hela bygget på
 * något som inte hade med ändringen att göra. Snapshoten gör drift-kontrollen
 * hermetisk; nätet används bara när en ny övning tillkommit (eller --refresh).
 */
const SNAPSHOT = path.join(ROOT, 'scripts/exercise-db-snapshot.json')
...
const CATALOG = [
  ...
  { slug: 'armhavningar', name: 'Armhävningar', category: 'Bröst', db: 'Pushups', aliases: ['push-ups', 'pushups', 'armhävning', 'armhävning med kroppsvikt'] },
  ...
  { slug: 'pull-ups', name: 'Pull-ups', category: 'Rygg', db: 'Pullups', aliases: ['pullups', 'räckhäv', 'pull ups'] },
  ...
  { slug: 'hantelrodd', name: 'Hantelrodd', category: 'Rygg', db: 'One-Arm_Dumbbell_Row', aliases: ['enarmsrodd', 'dumbbell row', 'enarms hantelrodd'] },
  ...
  { slug: 'utfallssteg', name: 'Utfallssteg', category: 'Ben', db: 'Barbell_Lunge', aliases: ['utfall', 'lunges', 'lunge', 'utfallsgång', 'utfallsgang'] },
  ...
  { slug: 'step-up', name: 'Step-up', category: 'Ben', db: 'Dumbbell_Step_Ups', aliases: ['stepup', 'uppsteg', 'step-ups', 'step ups'] },
  ...
  { slug: 'sidolyft', name: 'Sidolyft', category: 'Axlar', db: 'Side_Lateral_Raise', aliases: ['lateral raise', 'sidolyft hantlar', 'lateral raises', 'sidolyft med hantlar'] },
  ...
  // ── Core ───────────────────────────────────────────────────────────────
  { slug: 'plankan', name: 'Plankan', category: 'Core', db: 'Plank', log: 'bodyweight', aliases: ['planka', 'plank'] },
  { slug: 'sidoplanka', name: 'Sidoplanka', category: 'Core', db: 'Side_Bridge', log: 'bodyweight', aliases: ['side plank', 'sidplanka'] },
  { slug: 'sit-ups', name: 'Sit-ups', category: 'Core', db: 'Sit-Up', aliases: ['situps', 'magböj', 'sit ups'] },
  { slug: 'crunches', name: 'Crunches', category: 'Core', db: 'Crunches', log: 'bodyweight', aliases: ['crunch', 'magcrunch'] },
  { slug: 'cykelcrunch', name: 'Cykelcrunch', category: 'Core', db: 'Air_Bike', log: 'bodyweight', aliases: ['bicycle crunch', 'cykelmage'] },
  { slug: 'russian-twist', name: 'Russian twist', category: 'Core', db: 'Russian_Twist', log: 'bodyweight', aliases: ['rysk twist'] },
  ...
  { slug: 'mountain-climbers', name: 'Mountain climbers', category: 'Core', db: 'Mountain_Climbers', log: 'bodyweight', aliases: ['bergsklättrare'] },
  { slug: 'dead-bug', name: 'Dead bug', category: 'Core', db: 'Dead_Bug', log: 'bodyweight', aliases: ['deadbug'] },
  ...
  // ── Kondition ──────────────────────────────────────────────────────────
  { slug: 'lopning', name: 'Löpning', category: 'Kondition', db: 'Running_Treadmill', log: 'time', aliases: ['löpband', 'running', 'löppass', 'jogging'] },
  { slug: 'gang', name: 'Gång', category: 'Kondition', db: 'Walking_Treadmill', log: 'time', aliases: ['promenad', 'walking'] },
  { slug: 'cykling', name: 'Cykling', category: 'Kondition', db: 'Bicycling', log: 'time', aliases: ['cykel', 'spinning', 'bike'] },
  { slug: 'roddmaskin', name: 'Roddmaskin', category: 'Kondition', db: 'Rowing_Stationary', log: 'time', aliases: ['rodd', 'rowing'] },
  { slug: 'crosstrainer', name: 'Crosstrainer', category: 'Kondition', db: 'Elliptical_Trainer', log: 'time', aliases: ['elliptical'] },
  { slug: 'stairmaster', name: 'Stairmaster', category: 'Kondition', db: 'Stairmaster', log: 'time', aliases: ['trappmaskin'] },
  { slug: 'hopprep', name: 'Hopprep', category: 'Kondition', db: 'Rope_Jumping', log: 'time', aliases: ['hoppa rep', 'jump rope'] },
...
/** Hämtar hela databasen med timeout och några omförsök. */
async function fetchDb() {
  let lastErr
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const res = await fetch(DB_URL, { signal: AbortSignal.timeout(30_000) })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      const json = await res.json()
      if (!Array.isArray(json) || json.length === 0) throw new Error('oväntat svar (ingen lista)')
      return json
    } catch (err) {
      lastErr = err
...
async function loadDb() {
  if (!REFRESH) {
    // Snapshoten först — den är incheckad och räcker så länge inga nya
    // övningar lagts till i CATALOG.
    if (existsSync(SNAPSHOT)) {
      const snap = JSON.parse(await readFile(SNAPSHOT, 'utf8'))
      const have = new Set(snap.map((e) => e.id))
      if (CATALOG.every((c) => have.has(c.db))) return { db: snap, fromSnapshot: true }
      console.log('Snapshoten saknar nya övningar — hämtar free-exercise-db.')
    }
    if (existsSync(CACHE)) return { db: JSON.parse(await readFile(CACHE, 'utf8')), fromSnapshot: false }
  }
  const json = await fetchDb()
  await writeFile(CACHE, JSON.stringify(json))
  return { db: json, fromSnapshot: false }
}

async function main() {
  const { db, fromSnapshot } = await loadDb()
  ...
  for (const item of CATALOG) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      // så katalogen kan regenereras även utan den installerad (alla nuvarande
      // bilder är incheckade). Saknas den ska felet säga vad som ska göras, inte
      // bara ERR_MODULE_NOT_FOUND.
      const sharp = await import('sharp')
        .then((m) => m.default)
        .catch(() => {
          throw new Error(
            `Nya bilder behöver optimeras (${item.name}) men "sharp" är inte installerat. Kör: npm i -D sharp`
          )
        })
...
`
  ...
  // Uppdatera snapshoten när datan kom från nätet/cachen, så nästa körning
  // (och CI) klarar sig utan nätverk. Den ska checkas in tillsammans med
  // katalogen — drift-kontrollen i CI fångar om den glöms bort.
  if (!fromSnapshot) await writeFile(SNAPSHOT, JSON.stringify(pruneDb(db), null, 2) + '\n')

  console.log(
    `✓ ${CATALOG.length} övningar, ${downloaded} nya bilder optimerade${fromSnapshot ? ' (snapshot, inget nätverk)' : ''}`
  )
