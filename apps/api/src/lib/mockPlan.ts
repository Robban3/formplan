...
import type { Exercise, FitnessProfile, NutritionDay, PlanDay, RestDay, WorkoutDay } from './types'

export type MockGoal = FitnessProfile['goal']

// Bygger en övning ur den kurerade katalogen så dev-planen har exakt samma form
// som en AI-genererad plan (giltigt exercise_id + kanoniskt svenskt namn).
...
interface GoalTemplate {
  workoutDays: number[]
  workouts: Record<number, WorkoutDay | RestDay>
  nutrition: Record<number, NutritionDay>
}
...
export function buildMockPlanDays(planId: string, goal: MockGoal = 'maintain'): Omit<PlanDay, 'id'>[] {
  const template = GOAL_TEMPLATES[goal] ?? GOAL_TEMPLATES.maintain
  const workoutSet = new Set(template.workoutDays)
  const rows: Omit<PlanDay, 'id'>[] = []

  for (let weekday = 1; weekday <= 7; weekday++) {
    rows.push({
      plan_id: planId,
      weekday,
      type: workoutSet.has(weekday) ? 'workout' : 'rest',
...
function buildWeekNutrition(
  ...
  const out: Record<number, NutritionDay> = {}
  for (let d = 1; d <= 7; d++) {
